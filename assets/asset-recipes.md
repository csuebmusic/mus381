...
**Source:** generated by [`../build/generate-orientation-sample.py`](../under-the-hood/build/generate-orientation-sample.py), which writes it to `assets/audio/module-02-week-02/orientation-sample.wav`. That copy is the source of truth; upload it to the server path above. Script details in [`../build/README.md`](../under-the-hood/build/README.md).

**Substitution:** any stereo sustained sound with an audible decay (~10–18 s) works: a long bowed note that fades, a struck rim recorded in stereo, a sampler-generated bell-like timbre. Avoid sounds with a hard ending (the lab's Step 5 depends on the decay being there for the fade to sit on). A substitute has to stay clearly audible at the 7-second mark.
