...
**Listening:** Two-part historical listening on one idea, that every choice which turns sound into a record is a compositional choice. Part one, recording as composition, is the field-recording and acousmatic tradition (Francisco López, Chris Watson, Hildegard Westerkamp), three pieces on a spectrum of how present the recordist is. Part two, mixing as composition, is the "Three ways to mix a record" guide (Bill Evans Trio, the Ronettes with Spector, Pablo and King Tubby). Module 3 also includes a peer listening assignment after midterm submission, where students browse and listen to each other's sample libraries and respond briefly. The format differs from Module 2's peer listening: students are listening to libraries (collections of curated sounds) rather than finished pieces, so the prompts are oriented toward what's in the libraries and what they'd do with it.
